...
/**
	Saves games[] values to a file
*/
let saveToFile = ()=>{
	// data to string
	let sendData = "";
	games.forEach(e=>{
		sendData += `[${e.Name}]{${e.State}}\n`;
	});
	// Send to save.php
	let json_object = {data: sendData, path:"nData.txt"};
	let xhrS = new XMLHttpRequest();
	xhrS.open('POST', "/gameList/save.php");
	xhrS.setRequestHeader('Content-Type', 'application/json');
	xhrS.send(JSON.stringify(json_object));
	setStatus(null, `Data saved!`);
}
/**
...
let loadFiltered = async (a, f)=>{
	...
	let toTable = (e)=>{
		...
		let td3_btn = document.createElement(`input`);
		// ID collumn
		td1.innerText = `${e.ID}.`;
		// NAME collumn
		...
		td3_btn.type = "button";
		td3_btn.value = "set";
		td3_btn.setAttribute("itemid", e.ID);
		td3_btn.onclick = ()=>{
			let np = "";
			// ask for new state until correct
			do{np = prompt("New state", `${e.State}`);}while(!/^[0-4]$/.exec(np));
			e.State = np;	// change data state
			document.querySelector(`.statechange[itemid='${e.ID}']`)
			.firstChild.data = e.State;	// change state w/o reloading the page
		}
		// add to DOM
		td3.append(td3_btn);
